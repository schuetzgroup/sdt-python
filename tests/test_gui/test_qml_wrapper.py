...
MyItem {
    ...
    property list<int> myIntList: [1, 2, 3]
    property string myStr: "bla"
    property double myFloat: 1.2
}
""")
    c.create()
    assert c.status_ == gui.Component.Status.Ready
    ...
    # QtQml.QQmlProperty(...).read() segfaults with PySide6 6.4.3
    # assert inst.myIntList == [1, 2, 3]
...
""")
    ...
    assert c.status_ == gui.Component.Status.Ready
    inst = c.instance_
    assert gui.getNotifySignal(inst, "color") is inst.colorChanged
