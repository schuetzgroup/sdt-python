...
class TestAffineTrafo:
    ...
    def test_affine_trafo_square(self, params, locs, result):
        ...
        t = chromatic._affine_trafo(params, locs)
    ...
    def test_affine_trafo_rect(self, params, locs, result):
        ...
        t = chromatic._affine_trafo(params[:-1, :], locs)
        np.testing.assert_allclose(t, result)


class TestChromaticCorrector:
    @pytest.fixture
    def coords(self):
    ...
    def test_pairs_from_signatures(self):
        ...
        res = chromatic.Corrector._pairs_from_signatures(
    ...
    def test_find_pairs_no_frame(self, dataframes, dataframe_pairs):
        """chromatic.Corrector.find_pairs, no "frame" column"""
        cc = chromatic.Corrector(dataframes[0], dataframes[1])
        cc.find_pairs()
    ...
    def test_find_pairs_multi_file(self, dataframes, dataframe_pairs):
        """chromatic.Corrector.find_pairs, list of input features"""
        cc = chromatic.Corrector([dataframes[0]]*2, [dataframes[1]]*2)
        cc.find_pairs()
        res = cc.pairs.sort_values(("channel1", "x")).reset_index(drop=True)
    ...
    def test_find_pairs_multi_frame(self, dataframes, dataframe_pairs):
        """chromatic.Corrector.find_pairs, multiple frames"""
        dfs = []
        for d in dataframes:
            d1 = d.copy()
            d["frame"] = 0
        ...
        cc = chromatic.Corrector(dfs[0], dfs[1])
        cc.find_pairs()
        res = cc.pairs.sort_values(("channel1", "x")).reset_index(drop=True)
        exp = pd.concat([dataframe_pairs]*2)
        exp = exp.sort_values(("channel1", "x")).reset_index(drop=True)
    ...
    def test_fit_parameters(self, dataframe_pairs, trafo):
        """chromatic.Corrector.fit_parameters"""
        cc = chromatic.Corrector(None, None)
        # Add outlier
    ...
    def test_determine_parameters(self, dataframes, dataframe_pairs, trafo):
        """chromatic.Corrector.determine_parameters"""
        cc = chromatic.Corrector(dataframes[0], dataframes[1])
    ...
    def test_determine_parameters_mirrored(self, dataframes, dataframe_pairs,
                                           trafo):
        """chromatic.Corrector.determine_parameters, mirrored 1st axis"""
    ...
    def test_call_dataframe(self, dataframes):
        ...
        cc = chromatic.Corrector(*dataframes)
        cc.determine_parameters()
    ...
    def test_save_load(self, save_fmt, trafo):
        """chromatic.Corrector: save to/load from binary file"""
        cc = chromatic.Corrector()
        ...
        with tempfile.TemporaryFile() as f:
            ...
            f.seek(0)
            cc_loaded_file = chromatic.Corrector.load(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cc_loaded_fname = chromatic.Corrector.load(
