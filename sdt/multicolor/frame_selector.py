...
class FrameSelector:
    ...
    E.g. if employing alternating excitation for a FRET experiment, this can
    be used to select only the donor or only the acceptor frames.
    ...
    @config.set_columns
    def __call__(self, data, which, renumber=False, columns={}):
    ...
    @config.set_columns
    def restore_frame_numbers(self, data, which, columns={}):
