...
def adaptive_thresh(img, block_size, c, smooth=5, method="gaussian"):
    ...
    method : {"gaussian", "mean"}, optional
        Adaptive method. Defaults to "gaussian" (i.e.,
...
def otsu_thresh(img, factor=1., smooth=5):
    ...
    if math.isclose(factor, 1.):
        return mask

    thresh2, mask = cv2.threshold(img, thresh * factor, 1, cv2.THRESH_BINARY)
    return mask.astype(bool)


def percentile_thresh(img, percentile, smooth=5):
