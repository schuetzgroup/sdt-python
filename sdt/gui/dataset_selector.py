...
class DatasetSelector(QtQuick.QQuickItem):
    ...
    def datasets(self, dsets: DatasetCollection):
        ...
        if self._datasets is not None:
            ...
            self._datasets.elementsChanged.disconnect(self._changeKey)
        ...
        if dsets is not None:
            ...
            dsets.elementsChanged.connect(self._changeKey)
    ...
    def specialDatasets(self, dsets: DatasetCollection):
        ...
        if self._specialDatasets is not None:
            ...
            self._specialDatasets.elementsChanged.disconnect(
        ...
        if dsets is not None:
            dsets.modelReset.connect(self._makeKeys)
            dsets.elementsChanged.connect(self._changeSpecialKey)
    ...
    def _changeKey(self, index: int, count: int, roles: Iterable[str]):
        ...
        This is a slot for ``datasets.elementsChanged``.
        """
        if "key" not in roles:
            return
        for i in range(index, index + count):
            self._keyList.set(i + self._specialDatasets.rowCount(),
                              self._datasets.getProperty(i, "key"))
    ...
    def _changeSpecialKey(self, index: int, count: int, roles: Iterable[str]):
        ...
        This is a slot for ``specialDatasets.elementsChanged``.
        """
        if "key" not in roles:
            return
        for i in range(index, index + count):
            self._keyList.set(i, self._specialDatasets.getProperty(i, "key"))
    ...
    def _insertSpecialKey(self, parent: QtCore.QModelIndex, first: int,
        ...
        for i in range(first, last + 1):
            self._keyList.insert(i,
                                 self._specialDatasets.getProperty(i, "key"))
